...
// ═══════════════════════════════════════════
//  音素發音對照表（PHONEME RESPELLING）
// ═══════════════════════════════════════════
// 問題背景：瀏覽器的 SpeechSynthesisUtterance 收到「單獨一個字母」
// （例如 "a"、"t"）時，一律會把它唸成「字母名稱」（A 唸成 /eɪ/、
// T 唸成 /tiː/），而不是該字母代表的「子音/短母音」音素。
// 這個對照表把容易被唸錯的子音字母，轉成 TTS 唸出來會接近正確
// 子音音的「近似拼寫」（例如 b → "buh"、t → "tuh"）。
// 母音的處理方式不同：長母音時字母名稱本身就是對的（自然發音規則
// 「母音說出自己的名字」），所以母音不在這個表裡統一處理，而是在
// 每個單字資料裡，針對「短母音」情境另外用 phonemesSpeak 欄位覆寫。
// ─── 子音對照表（CONSONANT_TTS）───
// 此表由使用者在 Windows Chrome/Edge 實機逐一測試確認，
// 並非單純理論推導；若日後要再調整，請先用診斷工具
// （tts_diagnostic.html）實測，避免回到憑空猜測的狀態。
//   - b,c,d,h,r："子音+uh" 已確認可用
//   - f,g,l,m,n,p,t,v,w："子音+uh" 唸起來像「子音+oo/u」，
//     改用"子音+ah" 後確認可用（f/p/t 仍稍重，但比原本更接近）
//   - j,s,y,z：無論哪種"子音+母音"拼法都唸不出該子音本身，
//     最終改用「真實短單字」(jet/sun/yes/zoo) 取代，
//     發音保證正確，缺點是稍微比其他子音長一點
//   - x：改用真實單字 "ax"（包含 /ks/ 尾音）
//   - q：少數情況才會單獨出現，沿用 "kwah" 系列、未單獨實測
const CONSONANT_TTS = {
  b:'buh', c:'kuh', d:'duh', f:'fah', g:'gah', h:'huh',
  j:'jet',  k:'kuh', l:'lah', m:'mah', n:'nah', p:'pah',
  q:'kwah', r:'ruh', s:'sun', t:'tah', v:'vah', w:'wah',
  x:'ax',  y:'yes', z:'zoo'
};

// 把單一音素 token 轉成適合丟給 TTS 的字串。
// override：若該單字資料提供了 phonemesSpeak[i]，一律優先採用
// （undefined/null 表示沒有覆寫；空字串 '' 表示「這個字母不發音，跳過」）
// 注意：陣列裡的 undefined 元素經過 JSON.stringify 後會變成 null
// （這是 JS 標準行為），所以這裡必須同時檢查 null 和 undefined，
// 否則「沒有覆寫」的位置會被誤判成「要唸 null 這個字串」。
function resolvePhonemeSpeech(token, override) {
  if (override !== undefined && override !== null) return override; // 可能是 '' (靜音) 或具體覆寫拼法
  const key = (token || '').toLowerCase();
  if (Object.prototype.hasOwnProperty.call(CONSONANT_TTS, key)) {
    return CONSONANT_TTS[key];
  }
  return token; // 母音／字母組合：交給瀏覽器預設規則處理
}
...
// speakOverrides：與 phonemes 一一對應的發音覆寫陣列（可省略）。
//   - undefined（沒提供）：該字母用 CONSONANT_TTS 表或原樣交給 TTS
//   - 空字串 ''：該字母不發音（靜音字母，例如 magic e 的 e）
//   - 其他字串：直接用這個字串取代原字母丟給 TTS
async function speakWordSegmented(word, cardIdx, phonemes, speakOverrides) {
  ...
  for (let i = 0; i < segs.length; i++) {
    const seg = segs[i];
    const override = speakOverrides ? speakOverrides[i] : undefined;
    const toSpeak = resolvePhonemeSpeech(seg, override);
    ...
    if (toSpeak !== '') {
      ...
      // 靜音字母（例如 magic e 的尾巴 e）：不發聲，只停留一下讓視覺強調生效
      await delay(250);
    }
...
// Speak a single phoneme sound（用於 soundBadge 大按鈕、音素小按鈕）
// override：若呼叫端已經算好「應該唸什麼」，可直接傳入字串覆寫
//   （'' 代表靜音，跳過不唸）
function speakPhoneme(ph, override) {
  if (!window.speechSynthesis) return;
  window.speechSynthesis.cancel();
  // 有些 soundBadge 是給人看的複合字串，例如 "i / ie / igh"、"ar / or"，
  // 這種字串如果整串塞給 TTS 唸出來會完全錯誤；
  // 沒有提供覆寫時，預設只取第一個「/」前的音來唸。
  let token = ph;
  if (override === undefined && typeof ph === 'string' && ph.indexOf('/') !== -1) {
    token = ph.split('/')[0].trim();
  }
  const toSpeak = resolvePhonemeSpeech(token, override);
  if (toSpeak === '') return; // 靜音，不發聲
  const u = new SpeechSynthesisUtterance(toSpeak);
...
function makeWordCard(w, globalIdx) {
  ...
  // phonemesSpeak：與 phonemes 一一對應的發音覆寫陣列（單字資料可選提供）。
  // 用來修正「字母被 TTS 唸成字母名稱」的問題：
  //   - 子音字母會自動透過 CONSONANT_TTS 表修正，通常不需要覆寫
  //   - 短母音、靜音字母（如 magic e 的 e）才需要在資料裡明確指定
  const speakOverrides = w.phonemesSpeak || null;
  const phonemeDisplay = phonemes
    ? `<div class="phoneme-row">${phonemes.map((p,i) => {
        const ov = speakOverrides ? speakOverrides[i] : undefined;
        const ovArg = ov !== undefined ? `,'${String(ov).replace(/'/g,"\\'")}'` : '';
        return `<button class="ph-btn" onclick="event.stopPropagation();speakPhoneme('${p.replace(/'/g,"\\'")}'${ovArg})">
  ...
  const clickFn = phonemes
    ? `speakWordSegmented('${w.word.replace(/'/g,"\\'")}', ${globalIdx}, ${JSON.stringify(phonemes)}, ${speakOverrides ? JSON.stringify(speakOverrides) : 'null'})`
    : `speakWord('${w.word.replace(/'/g,"\\'")}', ${globalIdx})`;

  return `
    <div class="word-card${w.note ? ' has-note' : ''}" id="wc-${globalIdx}"
         onclick='${clickFn.replace(/'/g, "&#39;")}' tabindex="0">
      <button class="sound-btn" onclick='event.stopPropagation();${clickFn.replace(/'/g, "&#39;")}'>🔊</button>
